...
func GetNVIDIADriverMount(pluginAddress string) (binding *kubecontainer.Mount, driver string, err error) {
	...
	if respErr != nil {
		err = fmt.Sprintf("Failed read GPU driver mount points from nvidia-docker-plugin: %v", respErr)
		return
	}
	if resp.StatusCode != http.StatusOK {
		err = fmt.Sprintf("Failed read GPU driver mount points from nvidia-docker-plugin response status = %s", resp.Status)
		return
	}
	...
	if derr := decoder.Decode(&args); derr != nil {
		err = fmt.Sprintf("Failed read nvidia-docker-plugin response: %v", derr)
		return
	}
